...
Scene loadScene(SceneType type, const std::filesystem::path& dataDir)
{
    ...
    std::filesystem::path debPath = dataPath / "debugtex.png";
    std::filesystem::path debTexPath = dataPath / "tex.png";
    std::filesystem::path path = dataPath / "default.png";
    Image tex(path);
    Image debugTex(debPath);
    Image debTex(debTexPath);
    switch (type) {
    case SingleTriangle: {
    ...
    case TextureQuad: {
        // Load a 3D model with a 2x2 texture quad
        auto subMeshes = loadMesh(dataDir / "quad.obj");
        subMeshes[0].material.kd = glm::vec3(1.0f);
        subMeshes[0].material.kdTexture = debTex;
        std::move(std::begin(subMeshes), std::end(subMeshes), std::back_inserter(scene.meshes));
        scene.lights.push_back(PointLight{ glm::vec3(-1, 1, -1), glm::vec3(1) });
    } break;
    case Cube: {
    ...
    case Teapot: {
        ...
        for (int i = 0; i < subMeshes.size(); i++) {
            subMeshes[i].material.kdTexture = tex;
        }
        std::move(std::begin(subMeshes), std::end(subMeshes), std::back_inserter(scene.meshes));
        scene.lights.push_back(PointLight { glm::vec3(-1, 1, -1), glm::vec3(1) });
